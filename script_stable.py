# script_stable.py - Improved version with stability enhancements
"""
Enhanced PEBBLE-DIAYN with fixes for training collapse:
1. Corrected avg_env_reward (0.9 → 1.8)
2. Increased relabel frequency (100k → 200k)
3. Added discriminator entropy monitoring
4. Added gradient clipping for discriminator
5. Added early warning system for training collapse
"""
import os
import time
import torch
...
class DIAYNTrainCallback(BaseCallback):
    """
    Trains discriminator with entropy monitoring and gradient clipping.
    """
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.update_freq == 0:
            ...
            if buffer_size > self.trajectory_length:
                sample_size = min(1024, buffer_size)
                ...
                if len(candidate_trajectories) >= 256:
                    ...
                    if len(entropy) > target_samples:
                        top_k_indices = np.argsort(entropy)[-target_samples:]
                    ...
                    # Early warning: if entropy drops too low too fast
                    if len(self.entropy_history) >= 50:
                        recent_entropy = np.mean(self.entropy_history[-10:])
                        if recent_entropy < 0.3:
                            print(f"\n⚠️  WARNING: Discriminator entropy very low ({recent_entropy:.3f}). Possible overfitting!")
        return True


class PebbleRelabelCallback(BaseCallback):
    """
    PEBBLE relabeling with stability monitoring.
    """
    def __init__(
        self,
        discriminator,
        relabel_freq: int = 200_000,  # Increased from 100k
        env_reward_weight: float = 0.1,
        avg_env_reward: float = 1.8,  # Corrected from 0.9
    ...
    def _on_step(self) -> bool:
        if self.n_calls > 0 and self.n_calls % self.relabel_freq == 0:
            print(f"\n{'='*60}")
            print(f"RELABELING at step {self.n_calls:,}")
            print(f"{'='*60}")
...
class CollapseMonitorCallback(BaseCallback):
    """
    Monitors training for collapse and issues warnings.
    """
        ...
        if self.n_calls % self.check_freq == 0:
            ...
            if hasattr(self.logger, 'name_to_value'):
                ...
                if ep_rew is not None:
                    self.reward_window.append(ep_rew)
                    if len(self.reward_window) > 10:
                        self.reward_window.pop(0)

                    # Detect collapse: sustained negative and declining rewards
                    if len(self.reward_window) >= 5:
                        recent_avg = np.mean(self.reward_window[-3:])
                        if recent_avg < -500:
                            print(f"\n{'='*60}")
                            print("❌ TRAINING COLLAPSE DETECTED!")
                            print(f"Recent average reward: {recent_avg:.1f}")
                            print("Consider stopping training and investigating.")
                            print(f"{'='*60}\n")
        return True
...
def run_pebble_diayn():
    ...
    relabel_freq = 200_000  # Increased for stability

    # Corrected reward weights
    env_reward_weight = 0.1
    avg_env_reward = 1.8  # Corrected from 0.9

    run_name = f"PEBBLE_DIAYN_STABLE_{int(time.time())}"
    log_dir = f"./logs/{run_name}"
    checkpoint_dir = f"./checkpoints/{run_name}"
    os.makedirs(checkpoint_dir, exist_ok=True)

    print(f"--- Starting PEBBLE-Enhanced DIAYN (Stable): {run_name} ---")
    print(f"Key Changes:")
    print(f"  - avg_env_reward: {avg_env_reward} (was 0.9)")
    print(f"  - relabel_freq: {relabel_freq:,} (was 100,000)")
    print(f"  - Added gradient clipping and collapse monitoring")
    print(f"---\n")
    ...
    # Wrapper with environment reward
    env = SkillWrapper(
        base_env,
        discriminator,
        num_skills=num_skills,
    ...
    callbacks = CallbackList([
        ...
        CollapseMonitorCallback(check_freq=10000),
